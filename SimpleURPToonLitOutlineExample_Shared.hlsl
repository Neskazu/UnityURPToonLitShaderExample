...
#pragma once
#pragma multi_compile_fragment _ _ADDITIONAL_LIGHTS _FORWARD_PLUS
...
ToonSurfaceData InitializeSurfaceData(Varyings input)
{
    ...
    //metallic
    float2 uvMG = TRANSFORM_TEX(input.uv, _MetallicGlossMap);
    half4 mg = SAMPLE_TEXTURE2D(_MetallicGlossMap, sampler_MetallicGlossMap, uvMG);
    output.metallic = mg.r * _Metallic;
    output.smoothness = mg.a;
    return output;
}
...
#endif
#if defined(USE_FORWARD_PLUS) || defined(_FORWARD_PLUS)
